	...
	for {
		...
		case protoMsg := <-session.RecvChan:
			...
			case cmd.PlayerEnterSceneNotify:
				...
				session.SendMsg(cmd.EnterSceneReadyReq, &proto.EnterSceneReadyReq{EnterSceneToken: ntf.EnterSceneToken}, 0)
			case cmd.EnterSceneReadyRsp:
				ntf := protoMsg.PayloadMessage.(*proto.EnterSceneReadyRsp)
				session.SendMsg(cmd.SceneInitFinishReq, &proto.SceneInitFinishReq{EnterSceneToken: ntf.EnterSceneToken}, 0)
			case cmd.SceneInitFinishRsp:
				ntf := protoMsg.PayloadMessage.(*proto.SceneInitFinishRsp)
				session.SendMsg(cmd.EnterSceneDoneReq, &proto.EnterSceneDoneReq{EnterSceneToken: ntf.EnterSceneToken}, 0)
			case cmd.EnterSceneDoneRsp:
				ntf := protoMsg.PayloadMessage.(*proto.EnterSceneDoneRsp)
				enterSceneDone = true
				sceneBeginTime = uint32(time.Now().UnixMilli())
				session.SendMsg(cmd.PostEnterSceneReq, &proto.PostEnterSceneReq{EnterSceneToken: ntf.EnterSceneToken}, 0)
				if config.GetConfig().Hk4eRobot.DosLoopLogin {
					session.Close()
					return
				}
			//发送改名请求
			session.SendMsg(cmd.SetPlayerNameReq, &proto.SetPlayerNameReq{NickName: config.GetConfig().Hk4eRobot.NickName}, 0)
		...
		case <-ticker.C:
			...
			if config.GetConfig().Hk4eRobot.ClientMoveEnable {
				if enterSceneDone {
					...
					session.SendMsg(cmd.UnionCmdNotify, unionCmdNotify, 0)
				}
			}
			if tickCounter%5 != 0 {
				continue
